...
def select_variable_subset(FUNCTION_CATALOG, total_variables = 12, favor_pipes = 2.5):
    ...
    if len(counterpart_variables) != len(off_variables):
        ...
        freq_adjustment = pd.DataFrame(off_df['var_category'].value_counts(normalize = True))
        freq_adjustment = freq_adjustment.reset_index().rename(columns = { 'proportion':'adj_val'})
