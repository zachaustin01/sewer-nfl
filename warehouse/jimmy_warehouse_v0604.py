...
def get_team_hhi(api_data, trailing_weeks = 5):
    ...
    player_yards = api_data[(api_data['play_type']=='run')|(api_data['play_type']=='pass')].groupby(['season','week','posteam','fantasy_player_name'], as_index=False)['yards_gained'].sum().rename(columns={'yards_gained':'player_yards_gained'})

    team_yards = api_data[(api_data['play_type']=='run')|(api_data['play_type']=='pass')].groupby(['season','week','posteam'], as_index=False)['yards_gained'].sum()
    ...
    merged_df = merged_df.groupby(['season','week','posteam'], as_index=False)['percent_team_yards_sq'].sum().sort_values(by=['season','posteam','week'])
...
def get_passing_hhi(api_data, trailing_weeks = 5):

    player_yards = api_data[(api_data['play_type']=='pass')].groupby(['season','week','posteam','fantasy_player_name'], as_index=False)['yards_gained'].sum().rename(columns={'yards_gained':'player_yards_gained'})

    team_yards = api_data[(api_data['play_type']=='pass')].groupby(['season','week','posteam'], as_index=False)['yards_gained'].sum()
    ...
    output_df = merged_df.assign(team_passing_HHI = merged_df.groupby(['season','posteam'], as_index=False)['percent_team_yards_sq'].rolling(trailing_weeks).mean().rename(columns={'percent_team_yards_sq':'HHI'})['HHI'])[['season','week','posteam','team_passing_HHI']]
...
def get_rush_hhi(api_data, trailing_weeks = 5):
    ...
    player_yards = api_data[(api_data['play_type']=='run')].groupby(['season','week','posteam','fantasy_player_name'], as_index=False)['yards_gained'].sum().rename(columns={'yards_gained':'player_yards_gained'})

    team_yards = api_data[(api_data['play_type']=='run')].groupby(['season','week','posteam'], as_index=False)['yards_gained'].sum()
    ...
    output_df = merged_df.assign(team_rushing_HHI = merged_df.groupby(['season','posteam'], as_index=False)['percent_team_yards_sq'].rolling(trailing_weeks).mean().rename(columns={'percent_team_yards_sq':'HHI'})['HHI'])[['season','week','posteam','team_rushing_HHI']]

    return(output_df.reset_index(drop=True).rename(columns={'posteam':'team'}))


# yards per pass on defense
