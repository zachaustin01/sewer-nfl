...
def pipe_turnover_propensity(api_data, next_gen_stats_pass, trailing_weeks = 5):
    ...
    dfs = [temp_pct_pass, temp_drives_in_turnover, temp_qb_aggr, temp_qb_comp_rate,
           temp_qb_hits_allowed_off]

    df_final = ft.reduce(lambda left, right: pd.merge(left, right, on=['season','week','team'],
                                                      how='left'), dfs)
...
def pipe_def_turnover_propensity(api_data, next_gen_stats_pass, next_gen_stats_rec,
                                 trailing_weeks = 5):
    ...
    dfs = [temp_def_epa_pass, temp_def_qb_aggr, temp_def_turnover, temp_def_cushion,
           temp_def_separation, temp_def_comp_rate, temp_def_qb_hits]
